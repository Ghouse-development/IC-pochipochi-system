/**
 * 内装データ移行スクリプト
 * interiorProductsComplete.ts → Supabase SQL
 */
...
const categorySlugMap = {
  ...
  '小上がり': 'koagari',
  'お風呂': 'bathroom',
  '点検口': 'inspection-hatch',
  '換気システム': 'ventilation',
  '壁': 'wall',
  '室内ドア': 'interior-door',
  '室内窓': 'interior-window',
  'アクセントパネル': 'accent-panel',
  '格子': 'lattice',
  'アクセントタイル': 'accent-tile',
  'インテリアカウンター': 'interior-counter',
  '玄関靴箱': 'shoe-cabinet',
  '玄関手洗い': 'entrance-washbasin',
  'インターホン': 'intercom',
  'エアコン': 'aircon',
  'スイッチ/コンセント': 'switch-outlet',
  'トイレ収納': 'toilet-storage',
  '物干し金物': 'laundry-hardware',
  '乾太くん': 'kantatsu-kun',
  'ダウンライト施工費': 'downlight-install',
  'スロップシンク': 'slop-sink',
  'アクセサリー': 'accessory',
  '洗濯パン': 'laundry-pan',
  '洗濯機混合水栓': 'washing-machine-faucet',
  'ナノバブル発生装置': 'nanobubble'
};

// ファイルを読み込む
const filePath = path.join(__dirname, '../src/data/interiorProductsComplete.ts');
const fileContent = fs.readFileSync(filePath, 'utf-8');

// TypeScriptの配列をJSONとしてパース可能な形式に変換
function parseTypeScriptArray(content) {
  // exportとimportを削除
  let cleaned = content
    .replace(/import.*?;/g, '')
    .replace(/export const \w+: \w+\[\] = /, '')
    .replace(/\/\/.*$/gm, '') // コメント削除
    .trim();
...
function extractVariants(str) {
  ...
  const variantRegex = /\{\s*id:\s*'([^']+)',\s*color:\s*'([^']+)',\s*colorCode:\s*'([^']+)'(?:,\s*modelNumber:\s*'([^']+)')?\s*\}/g;
...
`;
  ...
  products.forEach((p, idx) => {
    ...
    if (!categories.has(key)) {
      const slug = categorySlugMap[key] || `interior-${categories.size + 1}`;
      categories.set(key, {
        name: p.categoryName,
        slug: `interior-${slug}`,
...
VALUES ('${escapeSQL(name)}', '${cat.slug}', 'interior', '${escapeSQL(name)}', ${cat.order}, true, false)
...
// ファイル出力
const outputPath = path.join(__dirname, '../sql/009_seed_interior_complete.sql');
