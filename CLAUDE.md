...
1. **実行環境** - local / preview / prod のいずれか
2. **再現手順** - 3〜10手順で記載
3. **期待結果** - 何が起こるべきか
4. **証跡** - ログ / スクリーンショット / 動画パス（最低1つ）
5. **GATE-1〜6のPass/Fail** - 各ゲートの結果
6. **変更ファイル/関数一覧** - 何を変更したか
7. **影響範囲とロールバック手順** - 問題発生時の対応
...
### 10. PDFスタイルブックの読み込みルール（重要）

**PDFスタイルブック（FACADE Style Book、Interior Style Book等）を読み込む際は、ページ単位で処理すること。**

- PDFは1ページずつ読み込む（全ページ一括読み込みはしない）
- 各ページの情報を確実に抽出してから次のページへ進む
- ページ数が多いPDFは、必要なページ範囲を指定して読み込む
- 読み込んだ情報は必ずユーザーに確認を取ってからデータベースに登録

**理由:**
- 大きなPDFを一度に読み込むとメモリ不足やタイムアウトが発生する
- ページ単位で処理することで、情報の抽出精度が向上する
- 段階的に確認しながら進めることで、誤登録を防止できる
