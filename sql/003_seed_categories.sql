...
SELECT id, 'exterior', 'ニチハ（標準）', 'exterior-wall-nichiha-std', 'ニチハ社製外壁材（標準）', 1
FROM categories WHERE slug = 'exterior-wall'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'ニチハ Fu-ge PREMIUM', 'exterior-wall-nichiha-premium', 'ニチハ社製外壁材（プレミアム）', 2
FROM categories WHERE slug = 'exterior-wall'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'KMEW', 'exterior-wall-kmew', 'KMEW社製外壁材', 3
FROM categories WHERE slug = 'exterior-wall'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'IG工業', 'exterior-wall-ig', 'IG工業社製金属サイディング', 4
FROM categories WHERE slug = 'exterior-wall'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'AICA', 'exterior-wall-aica', 'AICA社製ジョリパット', 5
FROM categories WHERE slug = 'exterior-wall'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'SOLIDO', 'exterior-wall-solido', 'KMEW SOLIDO', 6
FROM categories WHERE slug = 'exterior-wall'
ON CONFLICT (slug) DO NOTHING;

-- ポーチサブカテゴリ
INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'モルタル', 'porch-mortar', 'モルタル金鏝抑え（標準）', 1
FROM categories WHERE slug = 'porch'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', '名古屋モザイク', 'porch-nagoya-mosaic', '名古屋モザイク工業社製タイル', 2
FROM categories WHERE slug = 'porch'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'LIXIL', 'porch-lixil', 'LIXIL社製タイル', 3
FROM categories WHERE slug = 'porch'
ON CONFLICT (slug) DO NOTHING;

-- 窓サブカテゴリ
INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'APW330', 'window-apw330', 'YKK AP APW330シリーズ（ペアガラス）', 1
FROM categories WHERE slug = 'window'
ON CONFLICT (slug) DO NOTHING;

INSERT INTO categories (parent_id, category_type, name, slug, description, display_order)
SELECT id, 'exterior', 'APW430', 'window-apw430', 'YKK AP APW430シリーズ（トリプルガラス）', 2
FROM categories WHERE slug = 'window'
ON CONFLICT (slug) DO NOTHING;
...
INSERT INTO categories (category_type, name, slug, description, display_order, is_required) VALUES
  ...
  ('equipment', '給湯器', 'water-heater', '給湯器・エコキュート', 5, true),
